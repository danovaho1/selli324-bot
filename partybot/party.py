...
class PartyCommands(commands.Cog):
    ...
    async def sitout1(self, ctx: fortnitepy.ext.commands.Context) -> None:
    ...
    async def bp(self, ctx: fortnitepy.ext.commands.Context, tier: int) -> None:
        ...
        await ctx.send(f'Установлен уровень боевого пропуска: {tier}.')
    ...
    async def level(self, ctx: fortnitepy.ext.commands.Context, banner_level: int) -> None:
        ...
        await ctx.send(f'Установлен уровень: {level}.')
    ...
    async def echo1(self, ctx: fortnitepy.ext.commands.Context, *, content: str) -> None:
    ...
    async def leave(self, ctx: fortnitepy.ext.commands.Context) -> None:
        ...
        print(self.bot.message % f'Выешел из лобби.')
    ...
    async def kick(self, ctx: fortnitepy.ext.commands.Context, *, epic_username: Optional[str] = None) -> None:
        ...
        if member is None:
            await ctx.send("Не удалось кикнуть игрока, он точно с вами?")
        else:
            try:
                await member.kick()
                await ctx.send(f"Кикнут игрок: {member.display_name}.")
                print(self.bot.message % f"Кикнут игрок: {member.display_name}")
            except fortnitepy.errors.Forbidden:
                await ctx.send(f"Не удалось кикнуть {member.display_name}, так как я не лидер.")
                print(crayons.red(self.bot.message % f"Не удалось кикнуть {member.display_name}, так как я не лидер."))
    ...
    async def promote(self, ctx: fortnitepy.ext.commands.Context, *, epic_username: Optional[str] = None) -> None:
        ...
        if member is None:
            await ctx.send("Не удалось скрыть игроков, вы точно в пати?")
        else:
            try:
                await member.promote()
                await ctx.send(f"Назначен лидер: {member.display_name}.")
                print(self.bot.message % f"Назначен лидер: {member.display_name}")
            except fortnitepy.errors.Forbidden:
                await ctx.send(f"Не удалось назначить лидера {member.display_name}, я не лидер.")
                print(crayons.red(self.bot.message % f"Не удалось назначить лидера {member.display_name}, я не лидер."))
    ...
    async def playlist_id1(self, ctx: fortnitepy.ext.commands.Context, playlist_: str) -> None:
        try:
    ...
    async def privacy1(self, ctx: fortnitepy.ext.commands.Context, privacy_type: str) -> None:
        try:
    ...
    async def matchmakingcode1(self, ctx: fortnitepy.ext.commands.Context, *, custom_matchmaking_key: str) -> None:
    ...
    async def match1(self, ctx: fortnitepy.ext.commands.Context, players: Union[str, int] = 0,
    ...
    async def lobby1(self, ctx: fortnitepy.ext.commands.Context) -> None:
    ...
    async def join(self, ctx: fortnitepy.ext.commands.Context, *, epic_username: Optional[str] = None) -> None:
        if epic_username is None:
            ...
            if user is not None:
                ...
                epic_friend = None
                await ctx.send(f'Не удалось подключиться к: {epic_username}.')

        if isinstance(epic_friend, fortnitepy.Friend):
            try:
                await epic_friend.join_party()
                await ctx.send(f'Подключение к игроку: {epic_friend.display_name}.')
            except fortnitepy.errors.Forbidden:
                await ctx.send('Не удалось подключиться, данная группа приватная.')
            except fortnitepy.errors.PartyError:
                await ctx.send('Вы точно в пати? А то я не могу подключиться.')
        else:
    ...
    async def friend1(self, ctx: fortnitepy.ext.commands.Context, *, epic_username: str) -> None:
    ...
    async def playlist1(self, ctx: fortnitepy.ext.commands.Context, *, playlist_name: str) -> None:
        try:
        ...
        if self.bot.party.me.leader:
            if party_member is not None:
                ...
                if member is not None:
                    ...
                    await ctx.send(f'Не удалось найти игроков: {party_member}.')
                    print(crayons.red(self.bot.message % f'Не удалось найти игроков: {party_member}.'))
            else:
                await self.bot.set_and_update_party_prop(
                    'Default:RawSquadAssignments_j', {
                ...
                print(self.bot.message % f'Все игроки скрыты.')
        else:
            await ctx.send("Не удалось всех скрыть, я не лидер.")
            print(crayons.red(self.bot.message % f"Не удалось всех скрыть, я не лидер."))
    ...
    async def justchattin1(self, ctx: fortnitepy.ext.commands.Context) -> None:
